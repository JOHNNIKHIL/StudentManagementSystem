...
class DbFunction{
	...
	function create_course($cshort,$cfull){
			...
			$db = Database::getInstance();
			$mysqli = $db->getConnection();
			$query = "insert into tbl_course(cshort,cfull)values(?,?)";
			$stmt= $mysqli->prepare($query);
			if(false===$stmt){
				...
				$stmt->bind_param('ss',$cshort,$cfull);
				$stmt->execute();
...
function register($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$board1,$board2,$roll1,$roll2,
   ...
   $query = "INSERT INTO `registration` (`course`, `subject`, `fname`, `mname`, `lname`, `gender`, `gname`, `ocp`,
					 ...
					 `padd`, `board`, `board1`,`roll`,`roll1`,`pyear`,`yop1`,`sub`,`sub1`,`marks`,`marks1`,
			...
			$stmt->bind_param('sssssssssssssssssssssssssssssss',
					...
					$email,$country,$state,$city,$padd,$board1,$board2,$roll1,$roll2,$pyear1,$pyear2,
...
function edit_course($cshort,$cfull,$id){
    ...
    $db = Database::getInstance();
	$mysqli = $db->getConnection();
	//echo $cshort.$cfull.$id;exit;
	$query = "update tbl_course set cshort=?,cfull=? where cid=?";
	$stmt= $mysqli->prepare($query);
...
function edit_subject($sub1,$sub2,$sub3,$id){

    $db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "update subject set sub1=?,sub2=? ,sub3=? where subid=?";
	$stmt= $mysqli->prepare($query);
	$stmt->bind_param('sssi',$sub1,$sub2,$sub3,$id);
	$stmt->execute();
    echo '<script>'; 
...
function edit_std($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
  ...
  // echo $id;exit;
	...
	$query = "update registration set course=?,subject=?,fname=?,mname=?,lname=?,gender=?,gname=?,ocp=?
              , income=?,category=?,pchal=?,nationality=?,mobno=?,emailid=?,country=?,state=?,dist=?
         	 ,padd=?,board=?,roll=?,pyear=?,sub=?,marks=?,fmarks=?,board1=?,roll1=?,yop1=?,sub1=?
	...
	$rc=$stmt->bind_param('ssssssssssssssssssssssssssssssi',$cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$board1,$board2,$roll1,$roll2,
				   $pyear1,$pyear2,$sub1,$sub2,$marks1,$marks2,$fmarks1,$fmarks2,$id);
